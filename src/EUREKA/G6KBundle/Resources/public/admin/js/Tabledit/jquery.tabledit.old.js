/*!
 * Tabledit v1.2.0 (https://github.com/markcell/jQuery-Tabledit)
    ...
    $.fn.Tabledit = function(options) {
        ...
        var Delete = {
            ...
            submit: function(td) {
                ...
                // Hide table row.
                // JA $(td).parent('tr').addClass(settings.mutedClass).find('.tabledit-toolbar button:not(.tabledit-restore-button)').attr('disabled', true);
                // Show restore button.
                $(td).find('.tabledit-restore-button').show();
                // Set last deleted row.
                $lastDeletedRow = $(td).parent('tr');
				$(td).parent('tr').remove(); // JA
            },
            confirm: function(td) {

(function (global) {
	...
	function Formatter (input, options, onComplete) {
		if (options.type == 'date') {
			Formatter.Date(input, options, onComplete);
		} else {
			Formatter.Number(input, options, onComplete);
		}
	}

	Formatter.Date = function(input, options, onComplete) {
		...
		date.addEventListener('keydown', function(e) {
			...
			if (key == 8 && start > 0) { // backspace
				var toDelete = value.charAt(start - 1);
				if (~sep.indexOf(toDelete)) {
					e.preventDefault();
				...
				(~numericKeys.indexOf(key) || ~numericPadKeys.indexOf(key))) {
				e.preventDefault();
				e.stopPropagation();
				var chr = ~numericPadKeys.indexOf(key) ?
		...
		function fixDate(value) {
			...
			var year = fixYear(parseInt(values[Y]));
			...
			var output = dates.map(function(v, i) {
				v = v.toString();
		...
		function fixYear(year) {
			if ((year + '').length < 3) {
				var now = new Date();
				year += (Math.floor(now.getFullYear() / 100) - 1) * 100;
				if (year + 100 <= now.getFullYear()) {
					year += 100;
				}
			}
			return year;
	...
	Formatter.Number = function(input, options, onComplete) {
		...
		function formatInput(input, options, eventType, onComplete) {
			...
			if (value !== "") {
				...
				if (~value.indexOf(options.decimalPoint)) {

...
(function (global) {
	...
	var listboxContainer = [
		...
		'	<div id="combobox-COMBOBOXID" class="field-group" role="combobox">',
		...
		'			<span class="icon icon-chevron-down" title="Ouvrir ou fermer la liste"></span>',
		'		</a>',
		'	</div>',
	...
	var listboxContainer3 = [
		'<div class="listbox form-group" tabindex="0">',
		'	<div id="combobox-COMBOBOXID" class="field-group input-group" role="combobox">',
		'		<div id="listbox-edit-COMBOBOXID" class="form-control" aria-autocomplete="none" aria-readonly="true" tabindex="-1">',
		'			<span class="listbox-edit" aria-live="assertive"> </span>',
		'		</div>',
		'		<a class="listbox-button input-group-addon" role="button" aria-haspopup="true" aria-controls="listbox-items-COMBOBOXID" tabindex="-1">',
	...
	function Listbox(target, options) {
		...
		switch (this.options.theme) {
			case 'bootstrap3':
				listbox = listboxContainer3.join("");
				break;
			case 'bootstrap4':
				listbox = listboxContainer4.join("");
				break;
			default:
				listbox = listboxContainer.join("");
		}
		listbox = listbox.replace(/COMBOBOXID/g, this.id + '');
		this.listbox = document.createElement('div');
		this.listbox.innerHTML = listbox;
		this.listbox = this.listbox.firstElementChild;
		for (var attr of this.target.attributes) {
			if (["id", "name", "class", "style", "value", "tabindex", "required", "aria-controls"].indexOf(attr.name) < 0) {
				self.listbox.setAttribute(attr.name, attr.value);
			}
		}
		this.listbox.style.position = 'relative';
		this.hideObject(this.listbox.querySelector('input'));
		this.label = this.target.closest('.field-container').querySelector("label[for=" + this.id + "]");
		if (this.label !== null) {
			this.label.setAttribute('for', 'combobox-' + this.id);
			if (! this.listbox.hasAttribute('aria-label') && this.label.hasAttribute('id')) {
				this.listbox.setAttribute('aria-labelledby', this.label.getAttribute('id')); 
			}
		}
		if (this.target.hasAttribute('required')) {
			this.listbox.setAttribute('aria-required', true); 
		}
		this.listbox.setAttribute('aria-controls', this.id); 
		if (this.target.hasAttribute('aria-label')) {
			this.listbox.setAttribute('aria-label', this.target.getAttribute('aria-label'));
		}
		this.listbox.classList.add(this.options.theme);
		this.keys = new keyCodes();

		this.edit = this.listbox.querySelector('.listbox-edit');
		this.button = this.listbox.querySelector('.listbox-button');
		this.list = this.listbox.querySelector('.listbox-items');
		var hasSelected  = false;
		for (var child of this.target.children) {
			if (child.tagName == 'OPTGROUP') {
				var li = document.createElement('li');
				li.classList.add('listbox-itemgroup');
				li.setAttribute('tabindex', '-1');
				li.innerHTML = child.getAttribute('label'); 
				li.setAttribute('id', 'itemgroup' + self.id + '-' + self.list.children().length);
				li.style.width = child.style.style.width;
				self.list.appendChild(li);
				child.querySelectorAll('option').forEach( opt => {
					hasSelected = self.populateList(opt, true) || hasSelected;
				});
			} else {
				hasSelected = self.populateList(child, false) || hasSelected;
			}
		}
		if (!hasSelected) {
			this.list.children[0].classList.add('selected');
			this.list.children[0].setAttribute('aria-selected', true);
		}
		this.group = this.listbox.querySelector('.field-group');
		this.input = this.listbox.querySelector('input');
		this.input.setAttribute('name', this.target.getAttribute('name'));
		if (this.target.closest('.field-group') === null) {
			this.target.replaceWith(this.listbox);
		} else {
			var inputGroup = this.target.closest('.field-group');
			if (inputGroup.closest('.form-group') !== null) {
				this.listbox.classList.remove('form-group');
			}
			inputGroup.replaceWith(this.listbox);
		}
		this.input.listbox = this;
	...
	Listbox.prototype.init = function() {
		...
		this.selected = this.initial = this.list.querySelector('.selected');
		this.list.setAttribute('aria-activedescendant', this.selected.getAttribute('id'));
	...
	Listbox.prototype.populateList = function(option, nested) {
		var selected  = false;
		var value = option.hasAttribute('value') ? option.getAttribute('value') : option.textContent;
		var li = document.createElement('li');
		...
		if (nested) {
			li.classList.add('listbox-item-nested');
		}
		...
		li.setAttribute('id', 'item' + this.id + '-' + this.list.children.length);
		li.style.width = option.style.width;
	...
	Listbox.prototype.bindEditboxHandlers = function() {
		...
		this.listbox.addEventListener('blur', function(e) {
			self.edit.style.outline = '0px';
			return self.handleListBlur(this, e);
		});
		this.listbox.addEventListener('focus', function(e) {
			self.edit.style.outline = 'thin dotted';
			return false;
		});
		var label = document.querySelector("label[for='combobox-" + this.id + "']");
		label && label.addEventListener('click', function (e) {
			self.target.dispatchEvent(new FocusEvent('focus', {
				view: window,
				bubbles: true,
				cancelable: true
			}));
	...
	Listbox.prototype.bindButtonHandlers = function() {
		var self = this;
		this.group.addEventListener('keydown', function(e) {
			return self.handleEditKeyDown(this, e);
		});
		this.group.addEventListener('keypress', function(e) {
	...
	Listbox.prototype.bindListboxHandlers = function() {
		var self = this;
		this.list.addEventListener('focus', function(e) {
			return self.handleListFocus(this, e);
		});
		this.list.addEventListener('blur', function(e) {
			return self.handleListBlur(this, e);
		});
	...
	Listbox.prototype.bindOptionsHandlers = function() {
		var self = this;
		// for (var item of this.list.children) {
			// item.addEventListener('keydown', function(e) {
				// return self.handleOptionKeyDown.bind(this, e);
			// });
			// item.addEventListener('keypress', function(e) {
				// return self.handleOptionKeyPress.bind(this, e);
			// });
		...
		this.list.addEventListener('keydown', function(e) {
			for (var item of self.list.children) {
				if (e.target === item) {
					return self.handleOptionKeyDown(item, e);
				}
			}
		});
		this.list.addEventListener('keypress', function(e) {
			for (var item of self.list.children) {
				if (e.target === item) {
					return self.handleOptionKeyPress(item, e);
				}
			}
		});
		this.list.addEventListener('click', function(e) {
			for (var item of self.list.children) {
				if (e.target === item) {
					return self.handleOptionClick(item, e);
				}
			}
		});
	...
	Listbox.prototype.isOpen = function() {
		if (this.list.getAttribute('aria-expanded') == 'true') {
			return true;
		}
		else {
			return false;
	...
	Listbox.prototype.closeList = function(restore) {
		var curOption = this.list.querySelector('.selected');
		if (restore == true) {
	...
	Listbox.prototype.openList = function(restore) {
		...
		if (restore == true) {
			if (this.selected === null) {
				...
				this.selectOption(this.list.children[0]);
			}
			curOption = this.selected;
			for (var item of this.list.children) {
				item.classList.remove('selected');
		...
		if (parseInt(this.list.children[0].style.width, 10) > parseInt(this.group.style.width, 10)) {
			this.list.style.width = this.list.children[0].style.width;
		} else {
			this.list.style.width = this.group.style.width;
			for (var item of this.list.children) {
				item.style.width = this.group.style.width;
			}
		}
		...
		if (roomAfter < listHeight && roomAfter < roomBefore) {
			this.list.style.top = (- listHeight) + 'px'; // show list above group
		} else {
	...
	Listbox.prototype.searchOption = function(fromOption) {
		...
		var found = null;
		if (search !== "") {
			for (var i = fromOption + 1; i < this.list.children.length; i++) {
				var next = this.list.children[i];
	...
	Listbox.prototype.handleOptionKeyDown = function(id,  e) {
		...
		switch(e.keyCode) {
			case this.keys.tab: {
				...
				if (id.textContent != this.selected.textContent) {
			...
			case this.keys.up: {
				if (e.altKey) {
					...
					if (curNdx > 0) {
						var prev = this.list.children[curNdx - 1];

						// remove the selected class from the current selection
						id.classList.remove('selected');
						id.removeAttribute('aria-selected');
			...
			case this.keys.down: {
				if (e.altKey) {
					...
					if (curNdx < this.list.children.length - 1) {
						var next = this.list.children[curNdx + 1];
			...
			case this.keys.home: {
				...
				var first = this.list.children[0];

				// remove the selected class from the current selection
				this.list.children[curNdx].classList.remove('selected');
				this.list.children[curNdx].removeAttribute('aria-selected');
			...
			case this.keys.end: {
				...
				var last = this.list.children[this.list.children.length - 1];
	...
	Listbox.prototype.handleListBlur = function(id,  e) {
		...
		this.selectOption(self.list.querySelector('.selected'));
		// close the list box
		if (this.isOpen() === true) {
	...
	Listbox.prototype.theme = function(value) {
		if (value != null) {
			this.listbox.classList.remove(this.options.theme);
	...
	Listbox.prototype.reset = function() {
		...
		this.initial.classList.add('selected');
		this.initial.setAttribute('aria-selected', true);
	...
	Listbox.prototype.update = function() {
		...
		for (var child of this.list.children) {
			if (child.getAttribute('data-value') == val) {
				child.classList.add('selected');
				child.setAttribute('aria-selected', true);
				self.selected = child;
				self.edit.textContent = child.textContent;
				return false;
			}
		}
		return val;
	...
	Listbox.prototype.addItems = function(items) {
		...
		items.forEach( item => {
			...
			self.list.appendChild(li);
			li.setAttribute('id', 'item' + self.id + '-' + self.list.children.length);
		});
		var selected = self.list.querySelector('.selected');
		if (selected !== null) {
	...
	Listbox.prototype.showItem = function(value) {
		var self = this;
		if (value != null) {
			var item = self.list.querySelector("li[data-value=" + value + "]");
			if (item === null) {
				return; // already there
			}
			item = self.list.dataset['alb.item-' + value];
			if (!item) {
				return; // hideItem has not been used yet
			}
			var targetIndex = item.dataset['alb.pos'];
			var lastIndex = self.items.length - 1;
			if (lastIndex == -1) {
				self.list.insertBefore(item, self.list.firstChild);
			} else {
				self.items.forEach( (opt, i) => {
					if (opt.dataset['alb.pos'] > targetIndex) {
						opt.insertAdjacentElement('beforebegin', item);
						return false;
					} else if (i == lastIndex) {
						opt.insertAdjacentElement('afterend', item);
						return false;
					}
				});
			}
			self.selectOption(self.list.querySelector('.selected'));
	...
	Listbox.prototype.hideItem = function(value) {
		...
		if (value != null) {
			...
			if (!self.list.dataset['alb.itemsModified']) {
				// remember the order
				self.items.forEach( (item, i) => {
					item.dataset['alb.pos'] = i;
				});
			...
			self.list.dataset['alb.item-' + value] = item.parentElement.removeChild(item);
			self.selectOption(self.list.querySelector('.selected'));
			self.bindOptionsHandlers();
		}
	} // end hideItem
